    ...
    if(!display) {
        ...
        <div className="row mb-4 d-flex flex-row  bg-light gx-3 w-100">
            ...
            <div className="col-lg-6 col-8 align-content-center d-flex flex-wrap py-4">
                ...
                <Link to={`/headphones/${id}`} className="text-decoration-none text-white py-2 px-3 seepr">SEE PRODUCT</Link>
            </div>
        </div>
        </>
    }else {
...
const HeadPhones = function() {
    return <>
        <h1 className="p-5 w-100 text-center bg-dark text-white mb-4">HEADPHONES</h1>
        <section className="container-md all1 d-flex flex-wrap justify-content-center">
            {data && data.map(obj => {
                if(obj.type === "headphones") {
